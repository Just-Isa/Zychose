...
export class SceneManager {
    ...
    addTileToScene(objectKey: string, posX: number, posY: number, posZ: number, rotation: number) {
    ...
    createGrid() {
        for (let i = 0; i < this.streetArray.length; i++) {
            for (let j = 0; j < this.streetArray.length; j++) {
                let values = this.streetArray[i][j].split(':');
                let name = values[0];
                let rotation = Number(values[1]) * (Math.PI/180);
                if (isNaN(rotation)) rotation = 0;
                if (name != "") {
                    this.addTileToScene(name,(i-this.streetArray.length/2)*BOXSIZE,0,(j-this.streetArray.length/2)*BOXSIZE,rotation);
                } 
