<template>
  ...
  <div v-for="room in roomListItems" v-bind:key="room.roomNumber"
    ...
    <div class="flex justify-end items-center pr-3 text-lg">
      ...
      <button class="bg-button-blue-bright font-semibold rounded w-40 py-1 ml-3 mr-4">Upload Script</button>
      <button v-on:click="$router.push('/' + room.roomNumber)"
        class="bg-button-green font-semibold rounded w-16 py-1">Join</button>
    </div>
  </div>
</div>
</template>
