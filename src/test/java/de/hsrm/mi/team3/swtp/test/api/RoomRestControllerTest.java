...
public class RoomRestControllerTest {
    ...
    void restRouteRoomNumberReturnsRoomMap() throws Exception {
        ...
        roomService.updateRoom(roomBoxService.getSpecificRoom(FIRST_ROOM_NUMBER), "", MAP_JSON_STRING, "default-name",
                FIRST_ROOM_NUMBER, null);
    ...
    void restRouteChangeRoomOfUser() throws Exception {
        ...
        assertThat(roomBoxService.getSpecificRoom(SECOND_ROOM_NUMBER).getUserList()).size().isEqualTo(2);
    }
    ...
    void restRouteLogoutUserFromRoom() throws Exception {
        ...
        mockMvc.perform(
                post("/api/user/logout").contentType("text/plain")
                        .content("{\"sessionID:\"" + user.getSessionID() + "}"))
                .andExpect(status().isOk());
