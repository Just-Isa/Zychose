...
public class FileUploadRestControllerTest {
    ...
    static private final String MOCK_JYTHON_CONTENT = "print(\"test\")";
    static private final String REST_ROUTE = "/api/upload/1";

    static private final MockMultipartFile MOCK_JYTHON_FILE = new MockMultipartFile(
            "file",
            "mockFile.py",
            "text/plain",
            MOCK_JYTHON_CONTENT.getBytes());

    static private final int FIRST_ROOM_NUMBER = 1;
    ...
    void fileUploadPostTest() throws Exception {
        ...
        assertThat(roomBoxService.getSpecificRoom(FIRST_ROOM_NUMBER).getJythonScript()).isNotEmpty()
    ...
    void fileUploadGetTest() throws Exception {
        ...
        mockMvc.perform(multipart(REST_ROUTE).file(MOCK_JYTHON_FILE)).andExpect(status().isOk());
        mockMvc.perform(get(REST_ROUTE))
